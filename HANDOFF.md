...
_Last updated: 2026-07-08 (pt: **Web Designer/Publisher shell split built; page registry next.** Web Designer now owns the bundle-level design workspace without publish controls, and Web Publisher owns publish controls plus a first page-list/filter panel. The New page action intentionally stays disabled until a real page registry exists.)_

## ▶️ NEXT SESSION — Post-release backlog

- **P0 NEXT — Web Publisher page registry.** Current Web Publisher has a page panel, but it can only represent one implicit page derived from `web_sections`; `New page` is disabled because there is no `web_pages` entity yet. Next session: add a real page registry (schema/migration/backfill/default page), bind sections to page identity/slug, make `New page` create selectable pages, and scope preview/publish state per page so the publish UI always has a clear target.
...
**Web Designer/Publisher shell split (BUILT 2026-07-08, working tree):** Web Designer now renders a bundle-level workspace with page, asset-readiness, and publish-report context while removing the broken inline publish action. Web Publisher now renders the page-selection/filter panel above its publish controls, with `New page` disabled and explained until the next-session `web_pages` registry exists. Verification: focused component tests for both shells, `npx tsc --noEmit`, and in-app Browser checks for `/apps/relay-web-designer` and `/apps/relay-web-publisher`.
