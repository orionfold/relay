...
_Last updated: 2026-07-07 (pt: **App-detail row cache invalidation built; release still held.** This session checkpointed `gallery-card-interactions`, then completed `fix-app-detail-row-cache-invalidation`: row add/update/delete API mutations now revalidate scoped app-runtime cache tags for apps owning the changed table, no-op row updates skip cache churn, and the publish panel marks stale preview artifacts before publish. Local `main` remains ahead of `origin/main`; release is still intentionally paused. Continue app polish, then rerun release gates only when the operator chooses to cut.)_

## ▶️ NEXT SESSION — Continue app polish, not release cut

- **P0 START HERE — continue operator walkthrough app polish.** Remaining planned specs from `output/operator-walkthrough-feedback-2026-07-07.md`: `web-designer-site-controls`, `web-templates-pack`, `packs-first-ia`, `dashboard-settings-drilldowns`, `fix-in-app-preview-sheet-visibility`, and `flagship-card-polish`. Completed/built in this run: `publish-preview-ux-hardening`, `fix-turbopack-dynamic-transport-dispatch`, `fix-scheduled-lead-list-hygiene-dispatch`, `gallery-card-interactions`, `fix-app-detail-row-cache-invalidation`.
