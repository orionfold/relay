...
**Web Publisher page registry (BUILT 2026-07-08, this commit):** Added `web_pages` backing data plus default-page repair/backfill, bound `web_sections` to stable `pageSlug`, and scoped previews, publish requests, deployment status, and deployment listings by selected page. The Web Publisher pages list now creates pages, preserves selected page links, supports inline title rename with stable slugs, and exposes per-page delete with last-page protection. Verification: focused page-panel/publish/API/publisher vitest coverage, `npx tsc --noEmit`, `git diff --check`, and in-app Browser checks confirming shell order, create/select, delete visibility, and inline rename controls.

**Web Designer/Publisher shell split (BUILT 2026-07-08, working tree):** Web Designer now renders a bundle-level workspace with page, asset-readiness, and publish-report context while removing the broken inline publish action. Web Publisher gained the initial page-selection/filter panel above its publish controls; the follow-on page-registry pass now backs that panel with real pages. Verification: focused component tests for both shells, `npx tsc --noEmit`, and in-app Browser checks for `/apps/relay-web-designer` and `/apps/relay-web-publisher`.
