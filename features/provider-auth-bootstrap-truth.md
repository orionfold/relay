...
- With no stored preference, a usable Claude OAuth credential is preferred,
...
- A structurally usable global `~/.codex/auth.json` is reported only as
  `existingSessionAvailable`; it is not connected state.
