...
- **Model endpoint:** BYOK external API, private provider runtime or later hybrid
  tunnel.
- **Backup store:** customer-owned storage for G-082 authenticated, encrypted,
  versioned recovery bundles. The recovery key remains outside both Cell root
  and bundle destination; the first transport is an operator-selected local
  directory with provider adapters deferred.
...
| Snapshot/restore | Scheduler/UI/API/CLI | Relay → local/off-host recovery | G-082 uses snapshot v2, authenticated bundles, isolated drills and empty-root offline restore; live overwrite remains local-snapshot legacy only | `src/lib/snapshots/*`; `src/lib/recovery/*` |
| Secret storage | Settings/runtime/deploy configuration | Relay → secret root | G-082 carries the Cell keyfile only inside recovery ciphertext; customer recovery key stays separate; cloud KMS/reference later | `src/lib/utils/crypto.ts`; `src/lib/recovery/key.ts`; `src/lib/licensing/store.ts` |
...
| TM-007 | Storage attacker or operational failure | Bundle is public/tampered or recovery key is lost/stored beside it | Read ciphertext, attempt modification, or prevent recovery | Confidentiality breach if key also leaks; otherwise total data loss/unavailability | Customer data, recovery key, availability | G-082 snapshot v2 hashes, streamed AES-256-GCM with authenticated header, separate external key file, atomic publish, allowlisted extraction, SQLite integrity checks, isolated drill and empty-root restore (`snapshots/*`, `recovery/*`) | Provider access policy, KMS/escrow choice, automatic retention and SLA remain unimplemented/gated | Keep bundle/key separation explicit; add provider least privilege, versioned retention and periodic real off-Host drills before claims | Named create/verify/drill/restore receipts, age/source state, partial reconciliation and checksum/tag failures | Low-Medium locally; provider-dependent later | High | high |
...
| TM-011 | Stolen admin session or faulty upgrade/delete | Destructive action lacks step-up, verified recovery or compatibility check | Delete/replace/migrate data without safe rollback | Data corruption/loss and outage | Database/files, backups, availability | G-082 verifies recovery independently and refuses online/non-empty-root bundle restore; existing local restore takes a safety snapshot (`snapshot-manager.ts`, `recovery/orchestrator.ts`) | G-083/G-084 lifecycle reauthentication, exact diff and rollback orchestration remain | Require a recent verified G-082 checkpoint before Host lifecycle mutation; preserve offline export/recovery after lapse | Lifecycle audit, recovery age/compatibility reason codes and final inventory | Medium | High | high |
